...
class KucoinExchangeAccount(BaseExchangeAccount):
    ...
    def from_account_info(cls, name: str, account_info: dict):
        return cls(
            ...
            api_key_version=account_info.get("api_key_version", "2"),
    ...
    def __init__(
        ...
        self.api_key_version = str(api_key_version or "2").strip()
        if self.api_key_version not in {"2", "3"}:
            raise ValueError("KuCoin api_key_version must be 2 or 3")
        self.site_type = str(site_type or "global").strip() or "global"

    def _private_headers(self, method, endpoint, body="", timestamp_ms=None):
    ...
    def _request_sync(self, method, path, params=None):
        query = urlencode(params or {})
        endpoint = path + (f"?{query}" if query else "")
        request = urllib.request.Request(
            KUCOIN_API_BASE_URL + endpoint,
            headers=self._private_headers(method, endpoint),
            method=method.upper(),
        )
        try:
